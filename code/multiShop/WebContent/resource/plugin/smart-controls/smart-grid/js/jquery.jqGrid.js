...
//				this.curGbox.css({display:"block",left:x.clientX-6,top:y[1],height:y[2]});
				this.curGbox.css({display:"block",left:(x.clientX - $(ts.grid.bDiv).offset().left),top:y[1],height:y[2]});
...
//						this.curGbox.css({left:this.resizing.sOL+diff});
						this.curGbox.css({left:this.resizing.sOL+diff-$(ts.grid.bDiv).offset().left+6});
						if(p.forceFit===true ){
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			// yuxh 纵向位置
			var valign = cm.valign;
			if(ral) { result += "text-align:"+ral+";"; }
			if(valign) { result += "vertical-align:"+valign+";"; }
			...
			//屏蔽title ouyh
			result += (clas !== undefined ? (" class=\""+clas+"\"") :"") + ((cm.title && tv && false) ? (" title=\""+$.jgrid.stripHtml(tv)+"\"") :"");
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all smartGrid-pg-first'><span class='ui-icon ui-icon-seek-first'></span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all smartGrid-pg-prev'><span class='ui-icon ui-icon-seek-prev'></span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all smartGrid-pg-next'><span class='ui-icon ui-icon-seek-next'></span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all smartGrid-pg-last'><span class='ui-icon ui-icon-seek-end'></span></td>";
		...
		setColWidth = function () {
			...
			if(ts.p.shrinkToFit===true && vc > 0) {
				...
				// 原始宽度
				var oldWidthMap = {};
				var hasAutoExtendCol = false;
				$.each(ts.p.colModel, function(i) {
					if(this.hidden === false && !this.fixed){
						if (ts.p.autoExtendCol) {
							if (this.width) {
								oldWidthMap[this.name] = this.width;
							}
							
							if (ts.p.autoExtendCol == this.name) {
								hasAutoExtendCol = true;
							}
						}
				...
				// 自适应列
				if (ts.p.autoExtendCol && hasAutoExtendCol) {
					var autoExtendColWidth = initwidth;
					$.each(ts.p.colModel, function() {
						if (this.name != ts.p.autoExtendCol && oldWidthMap[this.name]) {
							this.width = oldWidthMap[this.name];
							autoExtendColWidth -= oldWidthMap[this.name];
						}
					});
					
					var autoExtendColWidthUsed = false;
					$.each(ts.p.colModel, function() {
						if (!autoExtendColWidthUsed && ts.p.autoExtendCol == this.name) {
							this.width = autoExtendColWidth;
							autoExtendColWidthUsed = true;
						}
					});
				}
				cr =0;
				...
				ts.p.colModel[lvc].width -= 1; // 减去1避免出现滚动条
		...
		if(this.p.multiselect) {
			this.p.colNames.unshift("<input role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox' style='margin-left: 14px;margin-top: 0px;'/>");
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			if(cSel === true) {
				...
				tdHtml = $(td).closest("td,th").attr('title') || $(td).closest("td,th").html();
		...
		return this.each(function(){
			...
			if(shrink===true) {
				...
				$.each($t.p.colModel, function(i) {
					if(this.hidden === false && !this.fixed){
						if ($t.p.autoExtendCol) {
							if (this.width) {
								oldWidthMap[this.name] = this.width;
							}
							
							if ($t.p.autoExtendCol == this.name) {
								hasAutoExtendCol = true;
							}
						}
				...
				// 自适应列
				if ($t.p.autoExtendCol && hasAutoExtendCol) {
					var autoExtendColWidth = initwidth;
					$.each($t.p.colModel, function() {
						if (this.name != $t.p.autoExtendCol && oldWidthMap[this.name]) {
							this.width = oldWidthMap[this.name];
							autoExtendColWidth -= oldWidthMap[this.name];
						}
					});
					...
					var autoExtendColWidthUsed = false;
					$.each($t.p.colModel, function() {
						if (!autoExtendColWidthUsed && $t.p.autoExtendCol == this.name) {
							this.width = autoExtendColWidth;
							autoExtendColWidthUsed = true;
						}
					});
				...
				$t.p.colModel[lvc].width -= 1; // 减去1避免出现滚动条
...
"use strict";	
	...
	$.unformat = function (cellval,options,pos,cnt) {
		...
		return ret !== undefined ? ret : cnt===true ? $(cellval).text() : $.jgrid.htmlDecode($(cellval).attr('title')) || $.jgrid.htmlDecode($(cellval).html());
	};
...
setSubGrid : function () {
	return this.each(function (){
		...
		suboptions = {
			...
			openicon: "ui-icon ui-icon-carat-1-sw",
			expandOnLoad:  false,
			delayOnLoad : 50,
			...
			if($(ts.rows[i]).hasClass('jqgrow')) {
				$(ts.rows[i].cells[pos]).bind('click', function() {
					...
					if($(this).hasClass("sgcollapsed")) {
						...
						$(this).html("<a style='cursor:pointer;'><i class=' "+ts.p.subGridOptions.plusicon+"'></i></a>").removeClass("sgexpanded").addClass("sgcollapsed");
